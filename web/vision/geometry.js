...
const nearestAngle=(current,target)=>current+Math.atan2(Math.sin(target-current),Math.cos(target-current));
let seed=3421;
...
function grain(kind){
  ...
  const t=new T.DataTexture(data,n,n);t.wrapS=t.wrapT=T.RepeatWrapping;t.repeat.set(kind==='cloth'?12:5,kind==='cloth'?12:5);
...
function thread(parent,mat,pts,r=.004,segments=42){return mesh(new T.TubeGeometry(curve(pts),segments,r,5,false),mat,parent);}
...
export {V,TAU,clamp,lerp,nearestAngle,rnd,grain,mesh,ell,curve,thread,surface,sweep,band,beads,ringPoints,mergeStatic};
