...
import json
import numpy as np
OUT=Path(__file__).resolve().parents[2]/'web/vision/skin'
...
def mesh_field(name, field, lo, hi, step):
    ...
    pos=pos[ind];nor=nor[ind];indices=inv.astype('<u4').reshape(-1,3)
    # Coincident grid-edge crossings occasionally make sub-picometre triangles.
    # Discard those empty faces while retaining the authored smooth normals.
    areas=np.linalg.norm(np.cross(pos[indices[:,1]]-pos[indices[:,0]],pos[indices[:,2]]-pos[indices[:,0]]),axis=1)*.5
    indices=indices[areas>=1e-12].reshape(-1)
